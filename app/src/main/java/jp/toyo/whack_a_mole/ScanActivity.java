...
import jp.toyo.whack_a_mole.adapter.DevicesAdapter;
import jp.toyo.whack_a_mole.adapter.DiscoveredBluetoothDevice;
import jp.toyo.whack_a_mole.util.Utils;
import jp.toyo.whack_a_mole.model.ScanStateLiveData;
import jp.toyo.whack_a_mole.model.ScanViewModel;
...
public class ScanActivity extends AppCompatActivity implements DevicesAdapter.OnItemClickListener {
    ...
    protected void onCreate(@Nullable final Bundle savedInstanceState) {
        ...
        mScanViewModel.getScannerState().observe(this, this::startScan);
        ...
        final DevicesAdapter adapter = new DevicesAdapter(this, mScanViewModel.getDevices());
    ...
    public boolean onCreateOptionsMenu(final Menu menu) {
        ...
        menu.findItem(R.id.filter_uuid).setChecked(mScanViewModel.isUuidFilterEnabled());
        menu.findItem(R.id.filter_nearby).setChecked(mScanViewModel.isNearbyFilterEnabled());
        return true;
    }
    ...
    public boolean onOptionsItemSelected(final MenuItem item) {
        switch (item.getItemId()) {
            case R.id.filter_uuid:
                ...
                mScanViewModel.filterByUuid(item.isChecked());
                return true;
            case R.id.filter_nearby:
                item.setChecked(!item.isChecked());
                mScanViewModel.filterByDistance(item.isChecked());
                return true;
        }
    ...
    public void onItemClick(@NonNull final DiscoveredBluetoothDevice device) {
        final Intent controlBlinkIntent = new Intent(this, ControlActivity.class);
        controlBlinkIntent.putExtra(ControlActivity.EXTRA_DEVICE, device);
     ...
     */
    private void startScan(final ScanStateLiveData state) {
    ...
    private void stopScan() {
        mScanViewModel.stopScan();
    }

    private void clear() {
        mScanViewModel.getDevices().clear();
        mScanViewModel.getScannerState().clearRecords();
    }
}